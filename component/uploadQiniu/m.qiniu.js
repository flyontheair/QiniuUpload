        ...
        this.uploader = function (op) {
            ...
            var getNewUpToken = function (file) {
                if (op.uptoken) {
                    ...
                    //Max-
                    op.uptoken_func(file,function(res){
                        if(res!='')
                            that.token = res;
                    });
            ...
            //Max-
            var mConfig = op.mConfig;
            //设置上传file的类型key
            var getUpType = function (file) {
                if (typeof mConfig != 'undefined') {
                    for (var item in mConfig) {
                        if (typeof mConfig[item].typeKey != 'undefined' && mConfig[item].typeKey!="") {
                            var key=","+mConfig[item].typeKey+",";
                            var names=file.name.split('.');
                            if(key.indexOf(names[names.length-1].toLowerCase())!=-1){
                                file.upType = item;
                            }
                        }
                        if (typeof file.upType != 'undefined') break;
                    }
                    if (typeof file.upType == 'undefined') file.upType = 'Others';
                }
            };
            ...
            uploader.bind('BeforeUpload', function (up, file) {
                ...
                //Max-，自定义设置
                if(up.settings && up.settings.chunk_size) up.settings.chunk_size=0;//设置成0有bug。手动置0
                if(typeof file.upType!='undefined'&&typeof mConfig[file.upType]!='undefined'&&typeof mConfig[file.upType].reOption!='undefined'){
                    up.setOption(mConfig[file.upType].reOption);
                }
                ...
                //    getNewUpToken(file);
                //}
                if (up.settings.get_new_uptoken) {
                    getNewUpToken(file);
                }
